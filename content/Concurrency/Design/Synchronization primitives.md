...
>[!quote] Analogy
>Let's say there's a hall full of workers, and many workers need to coordinate together to get work done, but they keep arguing and have a big heated discussion. 
>
>A chicken comes along and enforces a rule:
>
>>[!multi-column]
...
# 2. **Lock guards**
- Locks mutex created in a scope. Unlocks mutex once it leaves that **scope**.
- A crash? This primitive prevents creation of **deadlocks** 

> [!example]
> ```cpp
> std::mutex mutex_1
> {
> 	const std::lock_guard<std::mutex> lock(mutex_1); // locked here
> 	// work
> }
> // unlocked here
> ```
...
>[!analogy]
>The chicken grew conscious to how its being handled. The chicken only allows to be used only if they are on stage.
...
>
> ![[rubber-chicken (1).gif|200]]

---
## 4. **Atomics and Memory Barriers**
...
>
>The chicken comes and finds a cow to send a signal if the customer finishes writing their order. The cow looks unimpressed with the workers.
>
>![[Pasted image 20251016051651.png|300]]
>
...
# 7. **Read-Write Lock (Shared-Exclusive Lock)**
...
>
>>[!multi-column]
>>>![[Pasted image 20251016051811.png|100]]
>>
